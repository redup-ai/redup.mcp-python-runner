...
from redup_mcp_python_runner.errors import InputFilesError, ScriptMetadataError
...
class InputFileSpec(BaseModel):
    """One input file materialized under INPUTS_DIR for this call."""

    path: str = Field(
        description=(
            "Relative path under INPUTS_DIR (e.g. data.zip). "
            "No absolute paths or '..'."
        ),
    )
    content_base64: str = Field(
        description="File bytes as standard base64. Hosts may populate this field.",
    )


_FilesArg = Annotated[
    list[InputFileSpec] | None,
    Field(
        description=(
            "Optional input files for this call. Each item is written under "
            "INPUTS_DIR (env). Use Path(os.environ['INPUTS_DIR']) / path in code. "
            "Stateless: files exist only for this invocation."
        ),
    ),
]
...
def create_server(config: ServerConfig) -> FastMCP:
    ...
    async def execute_python(
        ...
        Stateless: each call uses a fresh temp directory. Only preinstalled
        packages are available (see ``check_environment``). Do not pass
        pip/uv/PEP 723 dependencies — they are rejected.

        Input files: pass ``files`` as ``[{path, content_base64}, …]``. Each
        file is written under ``INPUTS_DIR`` for this call only, e.g.::

            from pathlib import Path
            import os
            data = Path(os.environ["INPUTS_DIR"], "data.zip").read_bytes()

        Hosts may populate ``content_base64`` for you. Large payloads may hit
        caller transport size limits.

        Output binaries: write under ``ARTIFACTS_DIR`` (env), e.g.::

            Path(os.environ["ARTIFACTS_DIR"], "chart.png").write_bytes(png_bytes)

        Returns JSON: stdout, stderr, exit_code, duration_ms, timed_out,
        artifacts[{path, media_type, size, content_base64}].

        Args: ``code`` (required), ``timeout`` (seconds), ``files`` (optional).
        """
        async with tracked_work("execute_python"):
            ...
            if files:
                file_payloads = [
                    f.model_dump() if isinstance(f, InputFileSpec) else dict(f)
                    for f in files
                ]
            ...
            with tempfile.TemporaryDirectory(prefix="mcp-py-") as tmpdir:
                ...
                try:
                    materialize_input_files(
                        ...
                        max_total_bytes=config.max_inputs_total_bytes,
                        max_count=config.max_input_files,
                    )
    ...
    async def check_environment() -> str:
        ...
        async with tracked_work("check_environment"):
            ...
            lines = [
                ...
                f"Max input file: {config.max_input_file_bytes} bytes",
                f"Max input files: {config.max_input_files}",
                "Preinstalled packages (import these only):",
            ]
            for name in packages:
                lines.append(f"  - {name}")
            lines.append(
                "Input files: execute_python files[] → INPUTS_DIR; "
                "write outputs to ARTIFACTS_DIR → artifacts[]."
            )
            return "\n".join(lines)
